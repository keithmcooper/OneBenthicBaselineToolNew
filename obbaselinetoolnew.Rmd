...
all_poly <- rbind(raster_extent3,uk_eez3,agg_region,agg_siz3,agg_siz_hum_comb2,agg_siz_sc_comb2,agg_siz_ang_comb2 )
#head(all_poly)
#plot(all_poly)
#all_poly <- poly
...
selected_data <- merge_test[which(merge_test$name=='NW Europe'|merge_test$name=='UK EEZ'|merge_test$name==input$regionInput),]#|merge_test$name==input$regionInput|merge_test$name==input$siteInput|merge_test$name=='A Footprint'
#head(selected_data)
...
p <- ggplot(data=selected_data3(), aes(x=factor(Assemblages), y=n, fill=factor(Assemblages))) +
  ...
  #facet_wrap(~factor(name,levels=c('NW Europe','UK EEZ','South Coast RSMP','SC Footprint','127')),scales="free")+
  #facet_wrap(~factor(name,levels=c('NW Europe','UK EEZ','Humber RSMP','H Footprint','514/1')),scales="free")+
  facet_wrap(~factor(name,levels=c('NW Europe','UK EEZ','Humber RSMP','H Footprint','240')),scales="free")+
  facet_wrap(~factor(name),scales="free")+
  theme(legend.position = "none")+
  scale_y_continuous(labels = label_comma())
