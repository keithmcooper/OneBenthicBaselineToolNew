...
#### PROVIDE INPUT OPTIONS FOR ARRAYS AVAILABLE UNDER INITIAL INPUT #### 
  ...
  observeEvent(input$progInput,{
   ...
   ########################################        
           ## TEST TO ADD SELECTED EXTRACTION AREAS ADD SELECTION BOXES#### 25032022 
#selectInput(inputId="extractionareaInput", multiple = F,h4("Select Extraction Area",style="color:#808080"),choices =unique(agg$area_numbe))
   #############################        
```
...
#__________________________________________________________________________________________
  ...
  observeEvent(input$regionInput, { 
   ...
   leafletProxy("map1") %>%
      ...
      ############################
    ## TEST TO ADD SELECTED EXTRACTION AREAS #### 25032022
   # clearGroup("selected") %>%
     #addPolygons(data=agg[agg$area_numbe == input$extractionareaInput, ],color = "#444444", weight = 1, smoothFactor = 0.5,group = "selected",popup = paste0("<b>Name: </b>", agg$area_name, "<br>","<b>Number: </b>", agg$area_numbe))%>%
   # addPolygons(data=siz[siz$area_numbe == input$extractionareaInput, ],color = "orange", fill = FALSE,weight = 1.5, smoothFactor = 0.5,group = "selected",popup = paste0("<b>Name: </b>", siz$area_name, "<br>","<b>Number: </b>", siz$area_numbe))%>%
