...
crs(assemblages2) <- "+proj=longlat +datum=WGS84"
...
#pal_anglian <- c('#0000ee','#00ffff','#05aac1','#9a32cd','#00cd00','#9aff9a','#b40202','#ff0000','#ff8c00','#ffff00','#b4b404')
#plot(r_agg_region)
############################
...
leaflet() %>% 
    ...
    addRasterImage(r_agg_region,col=pal,opacity = 1,project = 3857)%>%
    
    ######################
  addLayersControl(
    overlayGroups = c("All (fauna)","All (phy)","Assemblages","euowf","owf","owf_cab","R4_chara","R4_bid","agg (PIZ)","agg (SIZ)","agg (REF)","disp","wave","tidal","tidal_cab","oga","mcz","sac","ncmpa"),options = layersControlOptions(collapsed = FALSE))%>%#"wave_cab",
    hideGroup(c("All (fauna)","All (phy)","Assemblages","euowf","owf","owf_cab","R4_chara","R4_bid","agg (PIZ)","agg (SIZ)","agg (REF)","disp","wave","tidal","tidal_cab","oga","mcz","sac","ncmpa"))%>%#
...
## Add selected PIZ/SIZ
    ...
    addLayersControl(
    overlayGroups = c("Selected (fauna)","Selected (phy)","All (fauna)","All (phy)","Assemblages","euowf","owf","owf_cab","R4_chara","R4_bid","agg (PIZ)","agg (SIZ)","agg (REF)","disp","wave","tidal","tidal_cab","oga","mcz","sac","ncmpa"),options = layersControlOptions(collapsed = FALSE))%>%#"wave_cab",
    hideGroup(c("Selected (phy)","All (fauna)","All (phy)","Assemblages","euowf","owf","owf_cab","R4_chara","R4_bid","agg (PIZ)","agg (SIZ)","agg (REF)","disp","wave","tidal","tidal_cab","oga","mcz","sac","ncmpa"))#
...
agg_siz_hum_comb$name <- 'H Footprint'# add in missing cols
agg_siz_hum_comb$id <- 'H Footprint'# add in missing cols
...
agg_siz_ang_comb$name <- 'A Footprint'# add in missing cols
agg_siz_ang_comb$id <- 'A Footprint'# add in missing cols
...
agg_siz_t_comb$name <- 'T Footprint'# add in missing cols
agg_siz_t_comb$id <- 'T Footprint'# add in missing cols
...
#plot(agg_siz_sc_comb)
agg_siz_sc_comb$name <- 'SC Footprint'# add in missing cols
agg_siz_sc_comb$id <- 'SC Footprint'# add in missing cols
...
#plot(agg_siz_sc_comb)
agg_siz_ec_comb$name <- 'EC Footprint'# add in missing cols
agg_siz_ec_comb$id <- 'EC Footprint'# add in missing cols
...
#plot(agg_siz_sc_comb)
agg_siz_nw457_comb$name <- 'NW(457) Footprint'# add in missing cols
agg_siz_nw457_comb$id <- 'NW(457) Footprint'# add in missing cols
...
#plot(agg_siz_sc_comb)
agg_siz_bc531_comb$name <- 'BC(531) Footprint'# add in missing cols
agg_siz_bc531_comb$id <- 'BC(531) Footprint'# add in missing cols
...
#plot(agg_siz_sc_comb)
agg_siz_bc476_comb$name <- 'BC(476) Footprint'# add in missing cols
agg_siz_bc476_comb$id <- 'BC(476) Footprint'# add in missing cols
...
#plot(agg_siz_sc_comb)
agg_siz_bc526_comb$name <- 'BC(526) Footprint'# add in missing cols
agg_siz_bc526_comb$id <- 'BC(526) Footprint'# add in missing cols
...
#plot(agg_siz_sc_comb)
agg_siz_gws_comb$name <- 'GWS Footprint'# add in missing cols
agg_siz_gws_comb$id <- 'GWS Footprint'# add in missing cols
...
#plot(agg_siz_sc_comb)
agg_siz_hs_comb$name <- 'HS Footprint'# add in missing cols
agg_siz_hs_comb$id <- 'HS Footprint'# add in missing cols
...
#plot(agg_siz_sc_comb)
agg_siz_bc472_comb$name <- 'BC(472) Footprint'# add in missing cols
agg_siz_bc472_comb$id <- 'BC(472) Footprint'# add in missing cols
...
#plot(agg_siz_sc_comb)
agg_siz_bc455_comb$name <- 'BC(455, 459, Bedwyn) Footprint'# add in missing cols
agg_siz_bc455_comb$id <- 'BC(455, 459, Bedwyn) Footprint'# add in missing cols
...
selected_data <- merge_test[which(merge_test$name=='NW Europe'|
                                    merge_test$name=='UK EEZ'|
...
r <- rast("C:/Users/kmc00/OneDrive - CEFAS/R_PROJECTS/OneBenthicBiodiversityLayersApp/R/www/Assemblages.tif")
agg_region2 <-  st_read(pool, query ="SELECT * from ap_marine_aggregate.extraction_regions;") 

region_rast2 <- reactive({
 
region_rast <-   agg_region2[which(agg_region2$stationsubgroup1 ==input$regionInput ),]#

return(region_rast)
})

sel_rast_poly2 <- reactive({
sel_rast_poly <- crop(r, region_rast2(), mask=TRUE)

## Make cluster a factor
values(sel_rast_poly) <- as.factor(values(sel_rast_poly))

return(sel_rast_poly)
})
...
ggplot() +
  geom_spatraster(data =sel_rast_poly2())+#r_ang_region
  scale_fill_manual(values = c('#0000ee','#00ffff','#05aac1','#9a32cd','#00cd00','#9aff9a','#b40202','#ff0000','#ff8c00','#ffff00','#b4b404','black'))
})
```
</div>
