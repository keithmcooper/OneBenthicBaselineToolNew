...
assemblages <- raster('www/Assemblages.tif')
assemblages = ratify(assemblages)
## Bring in assemblage tif
#r <- raster('www/AssemblageMaxClass_Apr22.tif')
#assemblages = ratify(r)
#assemblages.agg <- aggregate(assemblages, fact=2,fun = modal)
#assemblages<-terra::aggregate(r, fact = 2, fun = "modal", na.rm = TRUE)#
...
library(terra)
library(leaflet)
#assemblages2 <- rast('www/Assemblages.tif')
#assemblages2 = ratify(assemblages)
#assemblages2 = assemblages
...
r_uk_eez <- mask(assemblages,uk_eez)

library(sf)
agg_region <-  st_read(pool, query ="SELECT * from ap_marine_aggregate.extraction_regions;")
#pal2 <- c("#9999F8","#99FFFF","#9BDDE6","#D6ADEB","#99EB99","#D6FFD6","#E19999","#FF9999","#FFD199","#FFFF99","#E1E19A")
r_agg_region <- mask(assemblages, agg_region)

## Can't show SIZs footprint as raster has areas of corrupted data
agg_siz <-  st_read(pool, query ="SELECT * from ap_marine_aggregate.extraction_areas_siz where droped='FALSE';")# Read in data
agg_siz2 <- agg_siz[!duplicated(agg_siz$geom), ]
pal3 <- c("#9999F8","#99FFFF","#9BDDE6","#D6ADEB","#99EB99","#D6FFD6","#E19999","#FF9999","#FFFF99","#E1E19A")
r_agg_siz <- mask(assemblages, agg_siz2)
...
leaflet() %>% 
    ...
    addPolygons(data=siz,color = "#444444", fill = FALSE,weight = 1.5, smoothFactor = 0.5,group = "agg (SIZ)",popup = paste0("<b>Name: </b>", siz$area_name, "<br>","<b>Number: </b>", siz$area_numbe))%>%
    ...
    addPolygons(data=agg_region,color = "#444444", weight = 1,fillOpacity = 0.01, smoothFactor = 0.5,group = "Assemblages (Footprint)")%>%#,popup = paste0("<b>Name: </b>", agg_region$stationsubgroup1)
    ...
    addRasterImage(r_agg_region,col=pal2,opacity = 1,project = 3857,group = "Assemblages (RSMP Region)")%>%
     addRasterImage(r_agg_siz,col=pal3,opacity = 1,project = 3857,group = "Assemblages (Footprint)")%>%
    addRasterImage(r_uk_eez,col=pal2,opacity = 1,project = 3857,group = "Assemblages (EEZ)")%>%
    ######################
  addLayersControl(
    overlayGroups = c("All (fauna)","All (phy)","Assemblages","euowf","owf","owf_cab","R4_chara","R4_bid","agg (PIZ)","agg (SIZ)","agg (REF)","disp","wave","tidal","tidal_cab","oga","mcz","sac","ncmpa","Assemblages (All)","Assemblages (EEZ)","Assemblages (RSMP Region)","Assemblages (Footprint)"),options = layersControlOptions(collapsed = FALSE))%>%#"wave_cab",,"Assemblages (Footprint)"
    hideGroup(c("All (fauna)","All (phy)","Assemblages","euowf","owf","owf_cab","R4_chara","R4_bid","agg (PIZ)","agg (SIZ)","agg (REF)","disp","wave","tidal","tidal_cab","oga","mcz","sac","ncmpa","Assemblages (All)","Assemblages (EEZ)","Assemblages (RSMP Region)","Assemblages (Footprint)"))%>%#,"Assemblages (Footprint)"
...
## Add selected PIZ/SIZ
         ...
         addPolygons(data=siz[siz$area_numbe == input$siteInput, ],color = "#444444", fill = FALSE,weight = 1, smoothFactor = 0.5,group = "selected",popup = paste0("<b>Name: </b>", siz[siz$area_numbe == input$siteInput, ]$area_name, "<br>","<b>Number: </b>", siz[siz$area_numbe == input$siteInput, ]$area_numbe))%>%
    ...
    addLayersControl(
    overlayGroups = c("Selected (fauna)","Selected (phy)","All (fauna)","All (phy)","euowf","owf","owf_cab","R4_chara","R4_bid","agg (PIZ)","agg (SIZ)","agg (REF)","disp","wave","tidal","tidal_cab","oga","mcz","sac","ncmpa","Assemblages (All)","Assemblages (EEZ)","Assemblages (RSMP Region)","Assemblages (Footprint)"),options = layersControlOptions(collapsed = FALSE))%>%#,"Assemblages (Footprint)"
    hideGroup(c("Selected (phy)","All (fauna)","All (phy)","euowf","owf","owf_cab","R4_chara","R4_bid","agg (PIZ)","agg (SIZ)","agg (REF)","disp","wave","tidal","tidal_cab","oga","mcz","sac","ncmpa","Assemblages (All)","Assemblages (EEZ)","Assemblages (RSMP Region)","Assemblages (Footprint)"))#,"Assemblages (Footprint)"
#######################
#       addLayersControl(
#    overlayGroups = c("Region","Footprint"),options = layersControlOptions(collapsed = FALSE))%>%#"wave_cab",
# hideGroup(c("Region","Footprint"))#
...
map_overlays <- data.frame(
  ...
  Link=c(
    ...
    "NA",
     ...
     '<p><a href="https://www.marinedataexchange.co.uk/details/TCE-3573/2022-the-crown-estate-de-rijke-noordzee-cefas-flanders-marine-institute-offshore-wind-evidence-and-change-programme-north-sea-net-gain-project/summary"
      >Cooper, K. M., Downie, A.-L. and Curtis, M. (2022). North Sea Net Gain (NSNG). Cefas Project Report for The Crown Estate, 57 pp. Access the report</a></p>',
...
stations_in_siz <- st_join(st_points, siz, join =st_within)
   ...
   stations_in_siz2 <- stations_in_siz[,c(9,8,1,2,4,5,3,6,7,17,20)]
   colnames(stations_in_siz2) <- c("Survey","Station","Sample","Year","Longitude","Latitude","Gear","Cluster (fauna)","Cluster (phy)","Licence","Company")
...
r <- rast("www/Assemblages.tif")#_agg
...
## AGGREGATE SIZs
agg_siz <-  st_read(pool, query ="SELECT * from ap_marine_aggregate.extraction_areas_siz where droped='FALSE';")# Read in data
agg_siz <- agg_siz[!duplicated(agg_siz$area_numbe), ]
...
#class(agg_siz_hum2)
agg_siz_hum_comb <- agg_siz_hum2 %>%  summarise() %>%  mutate(region = sf::st_area(.))# union all polygons (not an obvious geometry merge tool, but quite effective)
...
agg_siz_ang_comb <- agg_siz_ang2 %>%  summarise() %>%  mutate(region = sf::st_area(.))# union all polygons (not an obvious geometry merge tool, but quite effective)
...
#class(agg_siz_sc2)
sf_use_s2(FALSE)#TURN SPHERICAL GEOMETRY OFF TO PREVENT ERROR
agg_siz_t_comb <- agg_siz_t2 %>%  summarise() %>%  mutate(region = sf::st_area(.))# union all polygons (not an obvious geometry merge tool, but quite effective)
...
#class(agg_siz_sc2)
sf_use_s2(FALSE)#TURN SPHERICAL GEOMETRY OFF TO PREVENT ERROR
agg_siz_sc_comb <- agg_siz_sc2 %>%  summarise() %>%  mutate(region = sf::st_area(.))# union all polygons (not an obvious geometry merge tool, but quite effective)
#plot(agg_siz_sc_comb)
...
#class(agg_siz_sc2)
sf_use_s2(FALSE)#TURN SPHERICAL GEOMETRY OFF TO PREVENT ERROR
agg_siz_ec_comb <- agg_siz_ec2 %>%  summarise() %>%  mutate(region = sf::st_area(.))# union all polygons (not an obvious geometry merge tool, but quite effective)
#plot(agg_siz_sc_comb)
...
#class(agg_siz_sc2)
sf_use_s2(FALSE)#TURN SPHERICAL GEOMETRY OFF TO PREVENT ERROR
agg_siz_nw457_comb <- agg_siz_nw4572 %>%  summarise() %>%  mutate(region = sf::st_area(.))# union all polygons (not an obvious geometry merge tool, but quite effective)
#plot(agg_siz_sc_comb)
...
#class(agg_siz_sc2)
sf_use_s2(FALSE)#TURN SPHERICAL GEOMETRY OFF TO PREVENT ERROR
agg_siz_bc531_comb <- agg_siz_bc5312 %>%  summarise() %>%  mutate(region = sf::st_area(.))# union all polygons (not an obvious geometry merge tool, but quite effective)
#plot(agg_siz_sc_comb)
...
#class(agg_siz_sc2)
sf_use_s2(FALSE)#TURN SPHERICAL GEOMETRY OFF TO PREVENT ERROR
agg_siz_bc476_comb <- agg_siz_bc4762 %>%  summarise() %>%  mutate(region = sf::st_area(.))# union all polygons (not an obvious geometry merge tool, but quite effective)
#plot(agg_siz_sc_comb)
...
#class(agg_siz_sc2)
sf_use_s2(FALSE)#TURN SPHERICAL GEOMETRY OFF TO PREVENT ERROR
agg_siz_bc526_comb <- agg_siz_bc5262 %>%  summarise() %>%  mutate(region = sf::st_area(.))# union all polygons (not an obvious geometry merge tool, but quite effective)
#plot(agg_siz_sc_comb)
...
#class(agg_siz_sc2)
sf_use_s2(FALSE)#TURN SPHERICAL GEOMETRY OFF TO PREVENT ERROR
agg_siz_gws_comb <- agg_siz_gws2 %>%  summarise() %>%  mutate(region = sf::st_area(.))# union all polygons (not an obvious geometry merge tool, but quite effective)
#plot(agg_siz_sc_comb)
...
#class(agg_siz_sc2)
sf_use_s2(FALSE)#TURN SPHERICAL GEOMETRY OFF TO PREVENT ERROR
agg_siz_hs_comb <- agg_siz_hs2 %>%  summarise() %>%  mutate(region = sf::st_area(.))# union all polygons (not an obvious geometry merge tool, but quite effective)
#plot(agg_siz_sc_comb)
...
#class(agg_siz_sc2)
sf_use_s2(FALSE)#TURN SPHERICAL GEOMETRY OFF TO PREVENT ERROR
agg_siz_bc472_comb <- agg_siz_bc4722 %>%  summarise() %>%  mutate(region = sf::st_area(.))# union all polygons (not an obvious geometry merge tool, but quite effective)
#plot(agg_siz_sc_comb)
...
#class(agg_siz_sc2)
sf_use_s2(FALSE)#TURN SPHERICAL GEOMETRY OFF TO PREVENT ERROR
agg_siz_bc455_comb <- agg_siz_bc4552 %>%  summarise() %>%  mutate(region = sf::st_area(.))# union all polygons (not an obvious geometry merge tool, but quite effective)
#plot(agg_siz_sc_comb)
...
#View(merge_test)
merge_test <- merge_test[,1:4]
#write.csv(merge_test,'C:\\Users\\kmc00\\OneDrive - CEFAS\\working\\526_yaxis.csv')
## Proxt search terms
...
p('The charts below show the area of seabed covered by different benthic faunal assemblages at a variety of spatial scales including:')
#br()
...
library(plotly)
p <- ggplot(data=selected_data3(), aes(x=factor(Assemblages), y=n, fill=factor(Assemblages))) +
  geom_bar(position="stack", stat="identity",na.rm = TRUE) +#,stat="identity"
  #geom_col(position = "fill") +#,stat="identity"
  #geom_label( x=3.5, aes(y=labelPosition, label=label), size=6) +
  #scale_fill_brewer(palette=4) +
  #Scale_fill_manual(values = c('#0000ee','#00ffff','#05aac1','#9a32cd','#00cd00','#9aff9a','#b40202','#ff0000','#ff8c00','#ffff00','#b4b404'))+
 ...
 facet_wrap(~factor(name,levels=level_order2()),scales="free")+#
  theme(legend.position = "none",
        plot.margin = margin(0.4, 0, 0.5,0.4, "cm"),#margin(t = 0, r = 0, b = 0, l = 0, unit = "pt")
